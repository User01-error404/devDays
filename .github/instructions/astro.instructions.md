...
- Define a `Props` interface in frontmatter for every reusable component and
  document the component contract with a short TSDoc/JSDoc comment. Explain
  what each prop controls, including optional/defaulted props and any
  constraints that are not obvious from the type.
...
- Comment intent and decisions, not mechanics. A comment should explain why a
  component needs a non-obvious structure or workaround, not repeat the
  markup or expression immediately below it.
- Keep comments current with the rendered behavior; update or delete an
  outdated comment in the same change that alters the related code.
